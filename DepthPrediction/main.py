...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss = criterion(depth_pred, target)
        optimizer.zero_grad()
    ...
    with open(train_csv, 'a') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writerow({'mse': avg.mse.item(), 'rmse': avg.rmse.item(), 'absrel': avg.absrel.item(), 'lg10': avg.lg10.item(),
             'mae': avg.mae.item(), 'delta1': avg.delta1.item(), 'delta2': avg.delta2.item(),
             'delta3': avg.delta3.item(), 'gpu_time': avg.gpu_time, 'data_time': avg.data_time})
...
def validate(val_loader, model, epoch, write_to_file=True):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        # measure accuracy and record loss
        result = Result()
        output1 = torch.index_select(depth_pred, 1, torch.LongTensor([0]).to(device))
        result.evaluate(output1, target)
        average_meter.update(result, gpu_time, data_time, input.size(0))
        end = time.time()
